...
def initialize_model_parallel(
    ...
    Args:
        ...
        pipeline_model_parallel_size (default 1):
            number of GPUs used to parallelize model pipeline.

        sequence_parallel_size (default 1):
            number of GPUs used to sequence parallel.
            there are two mode of sequence parallelism:
            megatron: run sequence parallel in tensor parallel rank
            deepspeed: is actually a long context windown parallelism
    ...
    Tp=2, PP=4. The present function will create 8 tensor model-parallel
    groups, 4 pipeline model-parallel groups and 8 data-parallel groups as:
    ...
    size2d = tensor_model_parallel_size * pipeline_model_parallel_size
    assert world_size % size2d == 0, f"{world_size} is not divisible by {size2d}"
    assert world_size % sequence_parallel_size == 0, f"world_size ({world_size}) is not divisible by sequence_parallel_size {sequence_parallel_size})"
    ...
    for i in range(num_model_parallel_groups):
        ranks = [data_parallel_group_ranks[i]
                 for data_parallel_group_ranks in model_parallel_group_ranks]
        group = torch.distributed.new_group(ranks)
        if rank in ranks:
